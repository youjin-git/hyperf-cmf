...
class AuthenticationController extends AbstractController
{
    ...
    public function lists()
    {
        ...
        $where = [];
        if($params['status'] !==''){
            $where['user_authentication.status'] = $params['status'];
        }
...
//            $where['uid'] = $uid;
//        }
        $lists =  $this->userAuthenticationModel
            ->leftJoin('user','user.id','=','user_authentication.uid')
            ->select('user_authentication.*','user.phone')
            ->where($where)
            ->orderBy('id','desc')
    ...
    public function change_status(){
        ...
        $this->userAuthenticationModel->where('id',$id)->update(['status'=>$status])?succ():err();
    }
