...
/**
 * @\Yj\Apidog\Annotation\ApiController(prefix="admin/vedio")
...
class VedioController extends AbstractController
{
    ...
    public function create(){
        ...
        $form =  Elm::createForm('admin/vedio/save');
        $form->setRule([
            \Yj\Form\Elm::YjUpload()->title('图片')->field('image'),
            \Yj\Form\Elm::YjFile()->title('视频')->field('video'),
            Elm::input('title', '标题')->required('标题必填'),
            Elm::input('link', '链接')->required('跳转必填'),
            Elm::hidden('id',$id)
        ]);

        $lists = $form->setTitle($id?'编辑视频':'添加视频')->formData($formData);
